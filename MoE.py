...
class MoE(nn.Module):
    ...
    def forward(self,x):    # x: (batch,seq_len,emb)
        ...
        # Noisy Top-K Gating，优化expert倾斜问题
        if self.training: # 仅训练时添加噪音
            noise=torch.randn_like(gate_prob)*nn.functional.softplus(self.noise(x)) # StandardNormal()*Softplus((x*W_noise))
        ...
        # 计算gate输出各expert的累计概率, 做一个loss让各累计概率尽量均衡，避免expert倾斜
        if self.training:
        ...
        return y.view(x_shape),gate_prob,importance_loss   # 返回gate的输出用于debug其均衡效果, 返回均衡loss 
